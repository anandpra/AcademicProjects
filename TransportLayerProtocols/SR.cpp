...
struct timerPacket* a_packedToBeSent;
int a_windowSize;
int a_BufferedPacketIndex;
int a_base;
int a_nextseqnum;
float a_timeout;
float a_static_timeout=8.0f;
struct timerPacket{
	float timeOffset;
	bool ackedFlag;
	bool retransmitFlag;
...
int b_base;
int b_windowSize;
map<int,struct pkt *> receiverBuffer;
 ...
 * If there is a retransmission for base packet, the timeout is set to default static value and estimateTimeout is called for estimation.
 * */
 ...
 * Sends packets to B as long as it lies inside the window.
 * Otherwise it stores in a buffer for transmission at a leter stage
 * */
...
{
	cout<<"*************************Inside A_output start*************************"<<endl;
	...
	senderPacket->timeOffset=0.0;
	senderPacket->ackedFlag=false;
	...
	if(a_nextseqnum >= a_base + a_windowSize){
		cout<<"A->Window size full. Can't send any more packets untill ack is received. Base Packet sequence num:"<<senderBuffer.at(a_base)->packet->seqnum<<endl;
	}else{
		sendPacketsinBulk();
	}
	a_totalPacketsSentFromApp++;
	cout<<"*************************Inside A_output end*************************"<<endl;
}
 ...
 * if a non corrupted packet is received from the sender which lies in the window, then
 * it is forwarded to the upper layer and the correspoding ack is sent back.
 * If this is the base packet, then the window is shifted to the right.
 * for packets in the last window it returns the corresponding acks.
 * */
void B_input(struct pkt packet)
{
	cout<<"*************************Inside B_Input start*************************"<<endl;
	...
	if(receiverBuffer.find(packet.seqnum)==receiverBuffer.end()){
		//this is a new packet
		if(packet.seqnum>=b_base&&packet.seqnum<b_base+b_windowSize){
			//This packet falls inside the window. Process it.
			*newPacket=packet;
			receiverBuffer[packet.seqnum]=newPacket;
			...
			fflush(stdout);
			//Trasferring packets to upper Layer in bulk
			int temp=b_base; bool flag=true; int count=0;
			while(receiverBuffer.find(temp)!=receiverBuffer.end()){
				if(flag){
					flag=false;
					cout<<"B->Transferring packets to upper Layer in bulk:"<<endl;
				}
				cout<<"B-> Packet with sequence number "<<receiverBuffer[temp]->seqnum<<" delievered to Upper layer"<<endl;
				tolayer5(1,receiverBuffer[temp]->payload);
				b_totalPacketsReceivedAtApp++;
				receiverBuffer.erase(temp);
				temp++;
				b_base++;
				count++;
			}
			if(count!=0){
				cout<<"B->Window shifted by "<<count<<endl;
			}
		}else if(packet.seqnum>=b_base-b_windowSize &&packet.seqnum<b_base){
			//This packet is also correctly received. But it has already transferred to the upper layer. Just send back the ack.
			struct pkt ackPacket;
			ackPacket.acknum=packet.seqnum;
			ackPacket.seqnum=packet.seqnum;
			ackPacket.checksum=checksum(&ackPacket);
			tolayer3(1,ackPacket);
			cout<<"B->Packet(Received in Last window) with sequence number:"<<packet.seqnum<<" Received."<<" Sending Ack packet with ACK number:"<<ackPacket.acknum<<". checksum:"<<ackPacket.checksum<<endl;
			//	cout<<"Num of Msg received from below:"<<b_counter<<endl;
			fflush(stdout);
		}else{
			cout<<"B-> Packet is outside the window. Ignoring it"<<endl;
		}

	}else{
		//This is a duplicate packet.Resend its corresponding Ack.
		struct pkt ackPacket;
		ackPacket.acknum=packet.seqnum;
		ackPacket.seqnum=packet.seqnum;
		ackPacket.checksum=checksum(&ackPacket);
		tolayer3(1,ackPacket);
		cout<<"B->Duplicate packet with sequence number:"<<packet.seqnum<<" Received."<<" Sending Ack packet with ACK number:"<<ackPacket.acknum<<". checksum:"<<ackPacket.checksum<<endl;
		//	cout<<"Num of Msg received from below:"<<b_counter<<endl;
		fflush(stdout);
	}

	cout<<"*************************Inside B_Input end*************************"<<endl;
}
