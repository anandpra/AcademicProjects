...
public class MostTrendingWordsLevelOneMapper extends
Mapper<LongWritable, Text, Text, LongWritable> {
	...
	public void map(LongWritable ikey, Text ivalue, Context context)
		...
		StringTokenizer itr;
		this.isHashTag=Boolean.parseBoolean(context.getConfiguration().get("isHashTag"));

		if(ivalue!=null &&ivalue.toString()!=null){
			String[] tweetData=ivalue.toString().split("~~");
			if(tweetData.length>=6){
				if(!isHashTag){
					itr = new StringTokenizer(tweetData[5]);
				}else{
					itr = new StringTokenizer(tweetData[6]);
				}
				String token;
				while (itr.hasMoreTokens()) {
					...
					String newToken;
					while(m.find()){
						newToken=token.substring(m.start(), m.end());
						token=token.replace(newToken,"");
						m= p.matcher(token);
						word.set(newToken);
						context.write(word, one);
					}
					if(!token.equals("")){
						token=applyRules(token);
	...
	private String applyRules(String token) {
		// TODO Auto-generated method stub
		//	token=token.replaceAll("([#@](.)*)", "");
		token=token.replaceAll("(http:(.)*)", "");
		if(token.contains("#")||token.contains("@")){
			token="";
			return token;
		}
		//Need to change this
		token=token.replaceAll("[\\$!&\"\'\\(\\)-:<>;([0-9]+)/\\?(\\*),%^\\[\\]\\{\\}~\\|_=`\\\\]", "");
		token=token.replaceAll(" ", "");
